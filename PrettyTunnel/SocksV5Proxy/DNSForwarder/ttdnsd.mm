...
int DNSServer::startDNSServer(const char* _localDNSIP,
                              ...
                              int _remoteSockProxyPort)
{
	...
	//create a new thread.
	serverOpt = [NSBlockOperation blockOperationWithBlock:^
	{
		DNSServer * dns_srv = DNSServer::getInstance();
		dns_srv->_start_server();
	}];
	[[NSOperationQueue globalQueue] addOperation:serverOpt];
...
int DNSServer::_start_server()
{
    ...
    for (;;)
    {
        ...
        for (i = 1; i < pfd_num; i++)
        {
            if (pfd[i].fd != -1 && ((pfd[i].revents & POLLIN) == POLLIN || (pfd[i].revents & POLLPRI) == POLLPRI || (pfd[i].revents & POLLOUT) == POLLOUT || (pfd[i].revents & POLLERR) == POLLERR))
            {
...
int DNSServer::_nonblocking_send(int fd, void* buff, int len)
{
    ...
    DDLogError(@"Send: %d of byte to write, %d of byte written\n", nByteToWrite, nByteWritten);
